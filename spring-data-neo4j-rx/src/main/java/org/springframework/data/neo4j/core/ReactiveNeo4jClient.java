...
public interface ReactiveNeo4jClient {
	 ...
	 */
	OngoingReactiveDelegation with(String targetDatabase);
	 ...
	 */
	@FunctionalInterface
	interface OngoingReactiveDelegation<T> {

		/**
		 * Delegates all things querying, parameter binding etc. to the {@link ReactiveStatementRunnerCallback callback}.
		 *
		 * @param callback
		 * @return
		 */
		Mono<T> delegateTo(ReactiveStatementRunnerCallback<T> callback);
	}
}
